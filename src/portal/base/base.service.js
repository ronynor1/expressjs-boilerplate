...
// find all
exports.CMSfindAll = async function (context, req_query, lang) {
  ...
  try {
    ...
    if(context._oneToManyModel)
    {
      ...
      if(context._oneToManyModel.search.length > 0)
      {
        ...
        // for admin, return only searched results
        language_options_obj['required'] = true;
      }
      else
      {
        // for admin, return null results if there arent any based on language
        language_options_obj['required'] = false;
      }
...
// find
exports.CMSfind = async function (context, id, lang) {
  ...
  try {
    ...
    if(context._oneToManyModel)
    {
      array.push({model: context._oneToManyModel.model, as: context._oneToManyModel.alias, where: {language: lang}, required: false });
    }
